...
class Gemma3:
    ...
    # ---------------- GGUF export ----------------
    def export_gguf(self, path, tokenizer: GemmaTokenizer):
        """
        Export to a GGUF file llama.cpp can load with the 'gemma3' architecture.
        ...
        def W(name):
            ...
            return xp.asnumpy(self.params[name]).T.astype(np.float32).copy()

        def B(name):
            return xp.asnumpy(self.params[name]).astype(np.float32).copy()

        writer.add_tensor("token_embd.weight", xp.asnumpy(self.params["embed_tokens"]).astype(np.float32))

        def Bnorm(name):
            return (xp.asnumpy(self.params[name]) + 1.0).astype(np.float32).copy()
    ...
    def train(self, sample_prompt="hello world"):
        ...
        while step_in_run < steps_target:
            ...
            if self.global_step % cfg.save_interval == 0 or step_in_run == steps_target:
                ...
                self.save_train_state(lr)
                self.model.export_gguf(os.path.join(cfg.checkpoint_dir, "model.gguf"), self.tok)
